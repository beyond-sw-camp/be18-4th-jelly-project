...
public class LotNumberServiceImpl implements LotNumberService {
    ...
    public PageResponse<LotNumberListResponse> searchLotNumbers(
            final SearchLotNumberRequest keyword, final Pageable pageable) {
        return PageResponse.from(
                lotNumberRepository.searchLots(keyword, pageable).map(LotNumberListResponse::fromEntity));
    }
    ...
    @Override
    @Transactional
    public LotNumberDetailResponse updateLotNumber(final Long lotNumberId, final UpdateLotNumberRequest request) {
        LotNumber lotNumber = findLotNumberById(lotNumberId);
    ...
    public LotNumber findLotNumberById(final Long lotNumberId) {
        ...
        return lotNumberRepository
                .findById(lotNumberId)
    ...
    public String generateLotNumberName(final Instant lotInstant, final Item item) {
        ...
        final String nameEnglishAndNumbers =
                item.getName().replaceAll("[^a-zA-Z0-9]", "").toUpperCase();
