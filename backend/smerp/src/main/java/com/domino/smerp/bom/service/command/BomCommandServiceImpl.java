...
public class BomCommandServiceImpl implements BomCommandService {
    ...
    public BomDetailResponse createBom(final CreateBomRequest request) {
        ...
        if (bomRepository.existsByParentItem_ItemIdAndChildItem_ItemId(
                request.getParentItemId(), request.getChildItemId())) {
        ...
        // 중복 BOM 관계 체크
        if (bomRepository.existsByParentItem_ItemIdAndChildItem_ItemId(parentItem.getItemId(), childItem.getItemId())) {
            throw new CustomException(ErrorCode.BOM_DUPLICATE_RELATIONSHIP);
        }
    ...
    @Override
    @Transactional
    public BomDetailResponse updateBomRelation(final Long bomId, final UpdateBomRelationRequest request) {
        ...
        if (bomClosureRepository.existsById_AncestorItemIdAndId_DescendantItemId(childItemId, newParentItemId)) {
            throw new CustomException(ErrorCode.BOM_CIRCULAR_REFERENCE);
        }
    ...
    public void forceDeleteBom(final Long bomId) {
        ...
        final List<Long> descendantItemIds =
                descendants.stream().map(BomClosure::getDescendantItemId).collect(Collectors.toList());
            ...
            for (final BomClosure ancestor : ancestors) {
                for (final BomClosure descendant : descendants) {
                    ...
                    bomClosureRepository.upsertBomClosure(
                            ancestor.getAncestorItemId(), descendant.getDescendantItemId(), depth);
                }
            }
