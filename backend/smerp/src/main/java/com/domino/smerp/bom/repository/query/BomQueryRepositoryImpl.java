...
public class BomQueryRepositoryImpl implements BomQueryRepository {
                ...
                .select(Projections.fields(
                        ...
                        ExpressionUtils.as(
                                JPAExpressions.select(bc.count())
                                        .from(bc)
                                        .join(child)
                                        .on(child.itemId.eq(bc.id.descendantItemId))
                                        .where(
    ...
    public List<Long> findAllBomAndOrphanItemIds() {
        ...
        final List<Long> bomItemIds =
                queryFactory.select(b.parentItem.itemId).from(b).fetch();

        bomItemIds.addAll(queryFactory.select(b.childItem.itemId).from(b).fetch());
        ...
        return queryFactory
                ...
                .from(i)
                .where(
                        i.itemId.in(bomItemIds).or(i.itemId.notIn(bomItemIds)) // orphan 포함
                        )
                .fetch();
    }
