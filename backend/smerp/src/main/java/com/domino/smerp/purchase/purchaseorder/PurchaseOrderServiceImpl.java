...
public class PurchaseOrderServiceImpl implements PurchaseOrderService {
    ...
    public PurchaseOrderCreateResponse createPurchaseOrder(final PurchaseOrderCreateRequest request) {
        ...
        RequestOrder requestOrder = requestOrderRepository
                .findById(request.getRoId())
                .orElseThrow(() -> new EntityNotFoundException("발주 전표를 조회할 수 없습니다."));

        // 자동 계산 로직
        BigDecimal qty =
                requestOrder.getItems().stream().map(item -> item.getQty()).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal inboundUnitPrice = requestOrder.getItems().stream()
        ...
        if (request.getDocumentNo() != null && !request.getDocumentNo().isBlank()) {
            ...
            int lastSeq = purchaseOrderRepository
            ...
            LocalDate docDate = request.getNewDocDate() != null ? request.getNewDocDate() : LocalDate.now();

            String dateString = docDate.format(DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            int lastSeq =
                    purchaseOrderRepository.findLastSequenceByDate(dateString).orElse(0);
    ...
    public PageResponse<PurchaseOrderGetListResponse> searchPurchaseOrdes(
            final SearchPurchaseOrderRequest keyword, final Pageable pageable) {
        return PageResponse.from(purchaseOrderRepository
                .searchPurchaseOrder(keyword, pageable)
                .map(PurchaseOrderGetListResponse::fromEntity));
    }
    ;
    ...
    public PurchaseOrderGetDetailResponse getPurchaseOrderById(final Long poId) {
        PurchaseOrder entity = purchaseOrderRepository
                .findByIdWithRequestOrderAndItems(poId)
    ...
    @Override
    @Transactional
    public PurchaseOrderUpdateResponse updatePurchaseOrder(final Long poId, final PurchaseOrderUpdateRequest request) {
        PurchaseOrder entity = purchaseOrderRepository
                .findById(poId)
    ...
    public PurchaseOrderDeleteResponse deletePurchaseOrder(final Long poId) {
        ...
        return PurchaseOrderDeleteResponse.builder().message("구매 전표가 삭제되었습니다.").build();
    }

    // ✅ 구매 현황
    @Override
    @Transactional(readOnly = true)
    public List<SummaryPurchaseOrderResponse> getSummaryPurchaseOrders(
