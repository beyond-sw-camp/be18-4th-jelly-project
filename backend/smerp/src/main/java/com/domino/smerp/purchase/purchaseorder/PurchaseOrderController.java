...
public class PurchaseOrderController {
    ...
    // ✅ 구매 상세 조회
    @GetMapping("/{poId}")
    public ResponseEntity<PurchaseOrderGetDetailResponse> getPurchaseOrder(@PathVariable final Long poId) {
    ...
    public ResponseEntity<PurchaseOrderUpdateResponse> updatePurchaseOrder(
            @PathVariable final Long poId, @RequestBody final PurchaseOrderUpdateRequest request) {
        return ResponseEntity.ok(purchaseOrderService.updatePurchaseOrder(poId, request));
    }

    // ✅ 구매 삭제 (소프트 삭제)
    @DeleteMapping("/{poId}")
    public ResponseEntity<PurchaseOrderDeleteResponse> deletePurchaseOrder(@PathVariable final Long poId) {
