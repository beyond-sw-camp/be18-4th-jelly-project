...
public class ProductionPlanServiceImpl implements ProductionPlanService {
    ...
    public PageResponse<SearchProductionPlanListResponse> searchProductionPlans(
        ...
        return PageResponse.from(productionPlanRepository
    ...
    @Override
    @Transactional
    public ProductionPlanResponse createProductionPlan(final CreateProductionPlanRequest createProductionPlanRequest) {
        ...
        User user =
                userRepository.findByName(createProductionPlanRequest.getName()).orElse(null);
    ...
    */
    ...
    public ProductionPlanResponse updateProductionPlan(
            final Long id, final UpdateProductionPlanRequest updateProductionPlanRequest) {
        ...
        if ((updateProductionPlanRequest.getQty() != null)
                && (updateProductionPlanRequest.getQty().compareTo(BigDecimal.ZERO)) < 0)
            throw new CustomException(ErrorCode.QTY_UNDER_ZERO);

        User user = updateProductionPlanRequest.getName() != null
                ? userRepository
                        .findByName(updateProductionPlanRequest.getName())
        ...
        ProductionPlan updatedProductionPlan = ProductionPlan.builder()
                ...
                .title(
                        updateProductionPlanRequest.getTitle() != null
                                ? updateProductionPlanRequest.getTitle()
                                : productionPlan.getTitle())
                .qty(
                        updateProductionPlanRequest.getQty() != null
                                ? updateProductionPlanRequest.getQty()
                                : productionPlan.getQty())
                .remark(
                        updateProductionPlanRequest.getRemark() != null
                                ? updateProductionPlanRequest.getRemark()
                                : productionPlan.getRemark())
                .status(
                        updateProductionPlanRequest.getStatus() != null
                                ? updateProductionPlanRequest.getStatus()
                                : productionPlan.getStatus())
                .user(user)
                .build();
    ...
    public void hardDeleteProductionPlan(final Long id) {
        ProductionPlan productionPlan = productionPlanRepository
                .findById(id)
                .orElseThrow(() -> new CustomException(ErrorCode.PRODUCTION_PLAN_NOT_FOUND));
        if (!productionPlan.isDeleted()) throw new CustomException(ErrorCode.PRODUCTION_PLAN_NOT_DELETED);
