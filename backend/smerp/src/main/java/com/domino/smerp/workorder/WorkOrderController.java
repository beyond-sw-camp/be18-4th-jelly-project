...
public class WorkOrderController {
    ...
    public ResponseEntity<PageResponse<SearchWorkOrderListResponse>> getLotNumbers(
            final @ModelAttribute SearchWorkOrderRequest keyword, final Pageable pageable) {
    ...
    public ResponseEntity<WorkOrderResponse> updateProductionOrder(
        ...
        WorkOrderResponse workOrderResponse = workOrderService.updateWorkOrder(workOrderId, updateWorkOrderRequest);
        return ResponseEntity.status(200).body(workOrderResponse);
    }

    // 삭제 - soft delete
    @DeleteMapping("/{work-order}")
    public ResponseEntity deleteProductionOrder(@PathVariable(name = "work-order") Long workOrderId) {
