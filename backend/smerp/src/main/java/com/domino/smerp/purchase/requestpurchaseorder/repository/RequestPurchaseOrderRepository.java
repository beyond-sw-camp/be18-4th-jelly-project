...
public interface RequestPurchaseOrderRepository
        extends JpaRepository<RequestPurchaseOrder, Long>, RequestPurchaseOrderQueryRepository {
    ...
    @Query("SELECT MAX(CAST(SUBSTRING(rpo.documentNo, 12) AS int)) " + "FROM RequestPurchaseOrder rpo "
