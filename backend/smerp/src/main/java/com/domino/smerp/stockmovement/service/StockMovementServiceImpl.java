...
public class StockMovementServiceImpl implements StockMovementService {
    ...
    public void createStockMovement(CreateStockMovementRequest createStockMovementRequest) {
        ...
        Item item = itemRepository
                .findById(createStockMovementRequest.getItemId())
                .orElseThrow(() -> new EntityNotFoundException("item not found by id"));

        User user = userRepository
                .findById(createStockMovementRequest.getUserId())
        ...
        if (movedQty.compareTo(BigDecimal.ZERO) < 0) {
            createInboundStockMovement(
                    item, movedQty, user, createStockMovementRequest.getTransactionType(), documentNo, totalQty);
        }
    ...
    public List<StockMovement> createInboundStockMovement(
        ...
        List<Stock> stocks = stockService.allocateStock(item.getItemId(), movedQty);

        List<StockMovement> stockMovements = new ArrayList<>();

        SrcDocType srcDocType = transactionType == TransactionType.INBOUND ? SrcDocType.PURCHASE : SrcDocType.NONE;

        String srcDocNo = documentNo;
        if (srcDocType.equals(SrcDocType.PURCHASE)) srcDocNo = documentNo + "PURCHASE";
    ...
    public List<StockMovement> createProduceStockMovement(WorkOrder workOrder) {
        ...
        List<Stock> stocks = stockService.allocateStock(workOrder.getItem().getItemId(), workOrder.getQty());
        ...
        String srcDocNo = (workOrder.getDocumentNo()) != null ? workOrder.getDocumentNo() + "PRODUCED" : "";
    ...
    */
    ...
    public StockMovementResponse createAdjustStockMovement(UpdateStockMovementRequest updateStockMovementRequest) {
        ...
        Item item = itemRepository
                .findByName(updateStockMovementRequest.getItemName())
                .orElseThrow(() -> new EntityNotFoundException("item not found by name"));

        User user = userRepository
                .findByName(updateStockMovementRequest.getUserName())
