...
public class StockMovement extends BaseEntity {
    ...
    @JoinColumn(name = "depart_warehouse_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
    private Warehouse departWarehouse;

    // 도착 창고
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "arrive_warehouse_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "lot_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
    private LotNumber lotNumber;

    // 생산 실적
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "pr_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
