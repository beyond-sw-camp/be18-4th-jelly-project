...
public class ProductionResultServiceImpl implements ProductionResultService {
    ...
    @Override
    @Transactional
    public ProductionResultResponse createProductionResult(
            final CreateProductionResultRequest createProductionResultRequest) {

        WorkOrder workOrder = workOrderRepository
                .findById(createProductionResultRequest.getWorkOrderId())
                .orElseThrow(() -> new EntityNotFoundException("work order not found by id"));

        Warehouse departWarehouse = warehouseRepository
                .findByName(createProductionResultRequest.getFactoryName())
                .orElseThrow(() -> new EntityNotFoundException("warehouse not found by id"));

        Item item = itemRepository
                .findByName(createProductionResultRequest.getItemName())
                .orElseThrow(() -> new EntityNotFoundException("item not found by name"));

        User user = userRepository
                .findByName(createProductionResultRequest.getUserName())
    ...
    public ProductionResult createProductionResultByWorkOrder(WorkOrder workOrder, BigDecimal producedQty) {
    ...
    public ProductionResultResponse updateProductionResult(
        ...
        ProductionResult updatedProductionResult = ProductionResult.builder()
                ...
                .user(
                        updateProductionResultRequest.getUserName() != null
                                ? userRepository
                                        .findByName(updateProductionResultRequest.getUserName())
                                        .orElseThrow(() -> new EntityNotFoundException("User not found by name"))
                                : productionResult.getUser())
                .documentNo(
                        updateProductionResultRequest.getDocumentNo() != null
                                ? updateProductionResultRequest.getDocumentNo()
                                : productionResult.getDocumentNo())
                .departWarehouse(
                        updateProductionResultRequest.getFactoryName() != null
                                ? warehouseRepository
                                        .findByName(updateProductionResultRequest.getFactoryName())
                                        .orElseThrow(() ->
                                                new EntityNotFoundException("Factory Warehouse not found by name"))
                                : productionResult.getDepartWarehouse())
                .workOrder(
                        updateProductionResultRequest.getWorkOrderId() != null
                                ? workOrderRepository
                                        .findById(updateProductionResultRequest.getWorkOrderId())
                                        .orElseThrow(() -> new EntityNotFoundException("WorkOrder not found by name"))
                                : productionResult.getWorkOrder())
                .item(
                        updateProductionResultRequest.getItemName() != null
                                ? itemRepository
                                        .findByName(updateProductionResultRequest.getItemName())
                                        .orElseThrow(() -> new EntityNotFoundException("Item not found by name"))
                                : productionResult.getItem())
                .qty(
                        updateProductionResultRequest.getQty() != null
    ...
    public void hardDeleteProductionResult(final Long id) {
        ProductionResult productionResult = productionResultRepository
                .findById(id)
                .orElseThrow(() -> new EntityNotFoundException("work order not found by id"));
        if (!productionResult.isDeleted()) throw new IllegalArgumentException("work order is not softly deleted");
