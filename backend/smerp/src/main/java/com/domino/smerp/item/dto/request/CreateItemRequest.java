...
public class CreateItemRequest {
    ...
    @NotNull(message = "품목 구분을 선택해주세요") @Range(min = 1, max = 6, message = "존재하지 않는 품목 구분입니다.")
    ...
    @NotNull(message = "입고 단가는 필수입니다.") @DecimalMin(value = "0.0", inclusive = true, message = "단가는 0.0이상이어야 합니다.")
    private final BigDecimal inboundUnitPrice;

    @NotNull(message = "출고 단가는 필수입니다.") @DecimalMin(value = "0.0", inclusive = true, message = "단가는 0.0이상이어야 합니다.")
    private final BigDecimal outboundUnitPrice;

    @NotBlank(message = "품목 사용 여부를 선택해주세요")
    private final String itemAct;

    @NotNull(message = "안전 재고 수량은 필수로 기입하셔야 합니다.") @Min(value = 0, message = "안전 재고 수량은 0이상이어야 합니다.")
