...
public class LotNumberController {
    ...
    // Lot.No 등록
    @PostMapping
    public ResponseEntity<LotNumberSimpleResponse> createLotNumber(
            final @Valid @RequestBody CreateLotNumberRequest request) {
    ...
    public ResponseEntity<PageResponse<LotNumberListResponse>> getLotNumbers(
            final @ModelAttribute SearchLotNumberRequest keyword, final Pageable pageable) {
    ...
    // Lot.No (이력)상세 조회
    @GetMapping("/{lots-id}")
    public ResponseEntity<LotNumberDetailResponse> getLotNumberById(@PathVariable("lots-id") final Long lotNumberId) {
        return ResponseEntity.ok(lotNumberService.getLotNumberById(lotNumberId));
    }

    // Lot.No 수정
    @PatchMapping("/{lots-id}")
    public ResponseEntity<LotNumberDetailResponse> updateLotNumber(
            @PathVariable("lots-id") final Long lotNumberId, final @Valid @RequestBody UpdateLotNumberRequest request) {
